...
public class Player extends GameObjects{
    ...
    public Player(Bitmap res, int w, int h, int numFrames){
        ...
        for (int i = 0; i <images.length ; i++) {
            images[i] = Bitmap.createBitmap(spriteSheet, 0, 0, width, height);
        }
    ...
    public void update(){
        ...
        Log.e("PlayerUp", "PlayerUp : " + up);
        if (up){
            dy = (int) (dya-=1.1);
            Log.e("PlayerUp", "true dy : " + dy);
        } else {
            dy = (int) (dya+=1.1);
            Log.e("PlayerUp", "false dy : " + dy);
        }
