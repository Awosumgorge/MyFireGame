...
public class BulletTwo {
    ...
    private GamePanel gamePanelView;
    ...
    public BulletTwo(GamePanel gamePanel, Bitmap bulletTwoBitMap, int x, int y, TargetBird targetBird, TargetBirdOne targetBirdOne ){

        this.gamePanelView = gamePanel;
        this.bulletTwoBitMap = bulletTwoBitMap;
        this.x = x;
        this.y = y;
    ...
    public void onDraw(Canvas canvas){
        this.updateBullet();
        canvas.drawBitmap(this.bulletTwoBitMap, this.x, this.y, null);
    }
    ...
    private void updateBullet(){
        //Log.e("BulletTwoTomNJerry", "GamePanel.bulletCount => " + GamePanel.bulletCount);
        //Log.e("BulletTwoTomNJerry", "this.gamePanelView.getBackgroundTotalWidth() => " + this.gamePanelView.getBackgroundTotalWidth());

        if(GamePanel.isFired){

            if ( this.x < this.gamePanelView.getBackgroundTotalWidth() && GamePanel.isFired){
                this.x += 20;
                //Log.e("BulletTwoTomNJerry", "BulletTwo  updateBullet function  TomNJerry X => " + this.x);
                //Log.e("BulletTwoTomNJerry", "BulletTwo  updateBullet function  this.gamePanelView.getBackgroundTotalWidth() => " + this.gamePanelView.getBackgroundTotalWidth()+"\n=======");
                isCollide();
                //isCollision();
                ...
                //if (this.x == this.gamePanelView.getBackgroundTotalWidth()){
                if (this.x == this.gamePanelView.getBackgroundTotalWidth() && GamePanel.bulletCount == 2 ){

                    //Log.e("BulletTwoTomNJerry", "BulletTwo  updateBullet GamePanel.bulletCount => " + GamePanel.bulletCount + " <= sadiq");
                    this.resetBullet = false; //for bullet Reset
                    GamePanel.isFired = false;//=setBulletFired(false);

                    //GamePanel.bulletCount = -1;
                    GamePanel.bulletTwo = null;
                }
            }
    ...
    private void isCollide(){
        if ( this.x  >= this.targetBird.getX()
            ...
            isCollision();
            Log.e("sadiq", "isCollide => targetBird hit success Bullet 2");
            ...
            isCollisionOne();
            Log.e("sadiq","isCollide => targetBirdOne hit success Bullet 2" );
        }
    }
